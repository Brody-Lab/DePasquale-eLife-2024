...
"""
function sep_neural_loglikelihood(x::Vector{T}, model::neuralDDM, data) where {T <: Real}
    
    @unpack θ,n,cross = model
    @unpack f = θ 
    
    model = neuralDDM(θ=θneural(x, f), n=n, cross=cross)
    
    sep_neural_loglikelihood(model, data)
...
"""
sep_neural_loglikelihood(model::neuralDDM, data) = sum(log.(vcat(vcat(sep_neural_likelihood(model, data)...)...)))
...
"""
function sep_neural_likelihood(model::neuralDDM, data)
